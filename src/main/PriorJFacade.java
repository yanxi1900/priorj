...
public class PriorJFacade {
	...
	/**
	 * This method save to priorj project directory all information about
	 * project opened.
	 * 
	 */
	...
	public void setJUnitVersion(String version) {
		...
		if (!version.toLowerCase().equals("junit3")
				&& !version.toLowerCase().equals("junit4"))
	...
	/**
	 * Running the prioritization with the technique Total Statement Coverage
	 * TSC.
	 * 
	 * @return The prioritized test suite.
	 */
	...
	/**
	 * Running the prioritization with the technique Additional Statement
	 * Coverage ASC.
	 * 
	 * @return The prioritized test suite.
	 */
	...
	/**
	 * Running the prioritization with the technique Additional Method Coverage
	 * ASC.
	 * 
	 * @return The prioritized test suite.
	 */
	...
	/**
	 * Running the prioritization with the Technique Total Echelon (Changed
	 * Blocks CB).
	 * 
	 * @param pathCodeNew
	 ...
	 * @return The prioritized test suite.
	 */
	public List<String> runPrioritizationCB(String pathCodeNew,
			String pathCodeOld) {
		try {
			return controllerPriorj.runPrioritizationCB(pathCodeNew,
					pathCodeOld);
		} catch (Exception e) {
			System.err.println(e.getMessage());
			return new ArrayList<String>();
	 ...
	 * 
	 * 1. "TMC" - Total Method Coverage 2. "TSC" - Total Statement Coverage 3.
	...
	public void addTechnique(String techniqueName) throws Exception {
		...
		if (techniqueName.isEmpty())
			throw new Exception("Empty Technique");

		if (!validateTechnique(techniqueName))
			throw new Exception("Invalid Technique");
	...
	public boolean validateTechnique(String technique) {
		List<String> list = Arrays.asList("tmc", "amc", "asc", "tsc", "rba",
				"cb", "rnd");

		return list.contains(technique);
	}
	...
	public void saveJavaPrioritizedSuite(List<String> tests, String technique) {
		...
		if (technique.equals("TMC"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.TOTAL_METHOD_COVERAGE);
		else if (technique.equals("TSC"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.TOTAL_STATEMENT_COVERAGE);
		else if (technique.equals("ASC"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.ADDITIONAL_STATEMENT_COVERAGE);
		else if (technique.equals("AMC"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.ADDITIONAL_METHOD_COVERAGE);
		else if (technique.equals("CB"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.CHANGED_BLOCKS_TOTAL);
		else if (technique.equals("RND"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.Random);
		else if (technique.equals("RBA"))
			controllerPriorj.saveJavaPrioritizedSuite(tests,
					TechniquesEnum.REFACTORING_BASED_APPROACH);
	...
	public List<String> generateSuiteSelection(String suiteName,String packageName, int size) {
		...
		List<List<String>> suites = controllerProject
				.openAllPrioritizedTestSuites(path);

		List<String> suiteNames = controllerProject.openSuitesNames();

		List<String> suiteSelection = controllerPriorj.generateSuiteSelection(
				suiteName, suites, suiteNames, packageName, size);
	 ...
	 * 
	 * @return A list of list with tests names.
	 */
	...
	public List<String> getAddedTechniquesNames() {
		return controllerPriorj.getAddedTechniques();
	}

	/**
	 * The Tree log execution.
	 * 
	 * @return A Jtree inside a scrollpane.
	 */
	 ...
	 * 
	 * @return A string with code tree trace.
	 */
	 ...
	 * @return
	 */
	public List<String> runRBARenameMethod(String pathApp, String className,
	 ...
	 *            The end line.
	 * 
	 * @return A list of method impacted.
	 */
	public List<String> runRBAExtractMethod(String pathApp,
			String originMethodName, String className, String newMethodName,
	...
	/**
	 * The total number of created projects.
	 * 
